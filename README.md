...
- `internal/controllers`：Controller 层，只处理 HTTP 输入输出，包括 path/query/body 读取、调用 service、返回 `response` 响应。
- `internal/services`：Service 层，放业务规则、输入 normalize、业务校验、分页 cursor 编解码。
- `internal/repository`：Repository 层，只处理数据库访问、DB 连接和数据库错误映射，不处理 HTTP。
- `internal/models`：Model 层，按领域放 request/input/filter/response/domain models。
