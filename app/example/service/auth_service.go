...
type AuthRepository interface {
	GetByEmail(ctx context.Context, email string) (types.AuthUser, error)
}
...
func (s *AuthService) Login(ctx context.Context, input types.LoginInput) (types.LoginResponse, error) {
	if !s.cfg.Enabled {
		return types.LoginResponse{}, apperr.BadRequest("auth_disabled", "auth is disabled")
	}
	if strings.TrimSpace(s.cfg.Secret) == "" {
		return types.LoginResponse{}, apperr.New(http.StatusServiceUnavailable, "auth_unavailable", "auth is unavailable")
	}
	if s.repo == nil {
		return types.LoginResponse{}, apperr.New(http.StatusServiceUnavailable, "auth_unavailable", "auth is unavailable")
	}
	...
	if !validEmail(email) || input.Password == "" {
		return types.LoginResponse{}, apperr.New(http.StatusUnauthorized, "invalid_credentials", "invalid email or password")
	}
...
func (s *AuthService) Logout(ctx context.Context, claims middleware.JWTClaims) error {
	_ = ctx
	...
	s.revoked[claims.JWTID] = claims.ExpiresAt
	return nil
}

func (s *AuthService) IsRevoked(claims middleware.JWTClaims) bool {
	if claims.JWTID == "" {
		return false
	}
	...
	s.cleanupRevokedLocked(now)
	expiresAt, ok := s.revoked[claims.JWTID]
	if !ok {
		return false
	}
	if !expiresAt.After(now) {
		delete(s.revoked, claims.JWTID)
		return false
	}
	return true
}

func (s *AuthService) signAccessToken(user types.AuthUser, now time.Time, expiresAt time.Time, roles []string, scopes []string) (string, error) {
